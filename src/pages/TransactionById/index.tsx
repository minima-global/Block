        ...
        <div className="content custom-scrollbar flex-grow text-white flex flex-col gap-4 pl-4 pr-5 py-6 lg:p-6">
          {data && active === 'DETAILS' && (
            <>
              ...
              <Block title="Parent" value={data?.header?.superparents[0].parent} link={`/t/${data?.header?.superparents[0].parent}`} />
              <h5 className="block mt-6 text-lg">Burn Transaction</h5>
              <Block title="Transaction Id" value={data.body.burntxn.transactionid} />
              ...
              {data.body.txnlist.length > 0 && (
                <>
                  ...
                  {data.body.txnlist.map((txn: any) => (
                    <Block key={txn} title="Transaction" value={txn} link={`/t/${txn}`} />
                  ))}
                </>
              )}
            </>
